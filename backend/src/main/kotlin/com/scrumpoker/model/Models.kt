 ...
 *
 * This file contains all the data classes, enums, and model definitions used throughout the Scrum
 * Poker backend application. These models define the structure of data as it flows through the
 * system and is stored in memory.
 *
 * Key Categories:
 ...
 *
 * These models ensure consistency between the backend business logic and the frontend client
 * communication.
 */
 ...
 *
 * This class models a user as they exist within the backend system. It contains all the information
 * needed to track a user's participation in a poker session, including their voting status and
 * estimate.
 *
 ...
 *
 * This class models a poker room where users gather to estimate story points. It maintains the list
 * of participants and the current session state.
 *
 * The room uses a mutable list of users to allow dynamic addition/removal of participants during
 * the session. The showEstimates flag controls whether estimates are visible to all participants.
 *
 ...
 *
 * This class represents the data sent by a client when requesting to join a poker room. It contains
 * the minimum information needed to create a user and add them to the specified room.
 *
 ...
 *
 * This class represents the data sent by a client when submitting their story point estimate. It
 * includes the user identification and their chosen estimate value.
 *
 ...
 *
 * This class represents the complete state of a poker room as sent to clients. It's used when
 * broadcasting room updates to all participants after any state change (user joins, votes,
 * estimates shown/hidden, etc.).
 *
 * The class transforms internal User objects into UserState objects that contain only the
 * information that should be visible to clients, respecting the estimate visibility rules.
 *
 ...
 *
 * This class represents user information as it should be displayed on the client. It's a filtered
 * version of the User class that respects visibility rules for estimates (estimates are only
 * included when showEstimates is true).
 *
 * This separation ensures that the backend can maintain full user state while controlling what
 * information is sent to clients based on the current session state.
 *
 ...
 *
 * This enum defines all the different types of messages that can be exchanged between the backend
 * and frontend via WebSocket connections. Each message type corresponds to a specific action or
 * event in the poker session workflow.
 *
 * Message Types:
 ...
 *
 * The enum values match exactly with the frontend MessageType enum to ensure consistent
 ...
 *
 * This class provides a consistent structure for all messages sent over WebSocket connections. It
 * acts as an envelope that contains the message type and the actual data payload.
 *
 * The payload can be any type of data (request objects, room states, etc.) and is
 * serialized/deserialized using Jackson JSON processing. The type field allows the receiver to
 * determine how to interpret the payload.
 *
